...
from rsac.collapse import extended_signature_from_seq, vectorized_extended_signature, all_bit_arrays
from rsac.sat import gen_random_kcnf, sat_bruteforce, eval_clause, unit_propagate, pure_literal_elim, simplify_with_assignment
from rsac.bucket_search import build_lut_basic, bucket_search_no_oracle, rsac_up_vectorized_search
...
def series_sat(ns, instances):
    ...
    for n in ns:
        ...
        for i in range(instances):
            ...
            clauses = gen_random_kcnf(n, rng.randint(int(1.2*n), int(1.8*n)), 3, rng)
    ...
    plt.xlabel('n'); plt.ylabel('Speedup (x)')
...
def series_sat_up(n, instances):
    ...
    for i in range(instances):
        ...
        _, rsac_checks, key, bucket = rsac_up_vectorized_search(cnf_reduced, fixed, rem_vars)
        rows.append({'n': n, 'fixed': len(fixed), 'remaining': len(rem_vars), 'bf_checks': bf_checks, 'rsac_checks': rsac_checks, 'bucket': bucket})

    import csv
    with open('data/sat_up_n{}.csv'.format(n), 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['n','fixed','remaining','bf_checks','rsac_checks','bucket'])
