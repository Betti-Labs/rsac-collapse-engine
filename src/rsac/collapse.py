...
def symbolic_reduction_loop(data, depth=16):
    ...
    for _ in range(depth):
        if len(current) < 2:
            break
        reduced = [
            digital_root(current[i] + current[i + 1]) for i in range(len(current) - 1)
        ]
        current = reduced
...
def all_bit_arrays(m: int) -> np.ndarray:
    ...
    for j in range(m):
        ...
        pattern = np.concatenate(
            [np.zeros(block, dtype=np.uint8), np.ones(block, dtype=np.uint8)]
        )
...
def vectorized_extended_signature(bits_mat: np.ndarray) -> list:
    ...
    for i in range(R):
        ...
        ant = (
            tuple(int(x) for x in antepenultimate[i])
            if antepenultimate is not None
            else tuple()
        )
        ent_tuple = (
            tuple(int(x) for x in ent_tail[i]) if ent_tail.shape[1] > 0 else tuple()
        )
