...
def generate_test_cases(
    n_vars_range=(3, 8), k_range=(2, 4), num_clauses_range=(5, 20), num_cases=50
):
    ...
    for _ in range(num_cases):
        ...
        test_cases.append(
            {"n_vars": n_vars, "k": k, "num_clauses": num_clauses, "clauses": clauses}
        )
...
def run_correctness_stress_test(test_cases, max_time_per_case=30):
    ...
    for i, case in enumerate(test_cases):
        print(
            f"Case {i+1}/{len(test_cases)}: n_vars={case['n_vars']}, k={case['k']}, clauses={case['num_clauses']}"
        )
        ...
        try:
            ...
            bs_start = time.time()
            bs_result = bucket_search_no_oracle(
                case["clauses"], case["n_vars"], eval_clause
            )
            ...
            if not correct:
                print(
                    f"  ERROR: Methods disagree! BF: {bf_result is not None}, BS: {bs_result is not None}"
                )
            else:
...
def analyze_results(correctness_results, performance_results, memory_results):
    ...
    if correctness_results:
        ...
        if correct_cases:
            speedups = [
                r["speedup"] for r in correct_cases if r["speedup"] != float("inf")
            ]
            if speedups:
    ...
    if performance_results:
        ...
        plt.tight_layout()
        plt.savefig(
            "stress_results/plots/performance_analysis.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.show()
    ...
    if memory_results:
        ...
        print(
            df_mem[
                ["n_vars", "lut_memory_mb", "arrays_memory_mb", "final_memory_mb"]
        ...
        plt.plot(
            df_mem["n_vars"], df_mem["arrays_memory_mb"], "ro-", label="Arrays Memory"
        )
        plt.plot(
            df_mem["n_vars"], df_mem["final_memory_mb"], "go-", label="Final Memory"
        )
        plt.xlabel("Number of Variables")
        ...
        plt.yscale("log")
        plt.savefig(
            "stress_results/plots/memory_analysis.png", dpi=150, bbox_inches="tight"
        )
        plt.show()
...
def main():
    ...
    parser.add_argument(
        "--correctness", action="store_true", help="Run correctness stress test"
    )
    parser.add_argument(
        "--performance", action="store_true", help="Run performance stress test"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of test cases for correctness test",
    )
    parser.add_argument(
        "--max-vars",
        type=int,
        default=10,
        help="Maximum number of variables for performance/memory tests",
    )
    parser.add_argument(
        "--trials", type=int, default=20, help="Number of trials per configuration"
    )
    ...
    if not any([args.correctness, args.performance, args.memory, args.all]):
        print(
            "Please specify which tests to run (--correctness, --performance, --memory, or --all)"
        )
        return
    ...
    if args.performance or args.all:
        ...
        performance_results = run_performance_stress_test(
            n_vars_range=(3, args.max_vars + 1), num_trials=args.trials
        )
