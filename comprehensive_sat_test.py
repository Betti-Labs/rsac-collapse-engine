...
def run_comprehensive_test(test_cases, max_vars=18):
    ...
    for i, test_case in enumerate(test_cases):
        ...
        try:
            start_time = time.time()
            rsac_result = bucket_search_no_oracle(test_case['clauses'], test_case['n_vars'], eval_clause)
            ...
            if bf_result is not None:
                correctness = 'CORRECT' if (bf_result is not None) == (rsac_assignment is not None) else 'INCORRECT'
            ...
            results.append({
                ...
                'bf_result': bf_status if bf_result is not None or bf_result == False else None,
                'bf_checks': bf_checks,
                'bf_time': bf_time,
...
def analyze_results(results):
    ...
    print("-" * 40)
    size_summary = df.groupby('n_vars').agg({
    ...
    print("\nTop 10 Speedups:")
    print("-" * 40)
    top_speedups = df.nlargest(10, 'speedup')[['name', 'n_vars', 'speedup', 'rsac_checks']]
    for _, row in top_speedups.iterrows():
        print(f"{row['name']:<25} {row['n_vars']:>3}v {row['speedup']:>8.1f}x {row['rsac_checks']:>6} checks")
