...
def create_small_test_instances():
    ...
    with open(instances_dir / "simple_sat_8.cnf", "w") as f:
        f.write(
            """c Simple satisfiable 8-variable 3-SAT instance
p cnf 8 12
1 2 3 0
-1 4 5 0
-2 -4 6 0
...
"""
    ...
    with open(instances_dir / "pigeonhole_3_2.cnf", "w") as f:
        f.write(
            """c Pigeonhole principle: 3 pigeons, 2 holes (unsatisfiable)
...
def print_summary(results):
    ...
    print(
        f"{'Instance':<20} {'Vars':<5} {'RSAC':<8} {'Checks':<8} {'Time(s)':<8} {'MiniSat':<8}"
    )
    print("-" * 60)

    for result in results:
        print(
            f"{result['instance']:<20} {result['n_vars']:<5} {result['rsac_result']:<8} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Run RSAC on SAT competition benchmarks"
    )
    parser.add_argument(
        "--max-vars", type=int, default=20, help="Maximum variables for RSAC"
    )
    parser.add_argument(
        "--download", action="store_true", help="Download external instances"
    )
    parser.add_argument(
        "--local-only", action="store_true", help="Only test local instances"
    )
